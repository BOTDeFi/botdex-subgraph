...
import {
  Bundle, Burn as BurnEvent, Factory, Mint as MintEvent, Pair, Swap as SwapEvent, Token, Transaction
} from "../../generated/schema";
import { Burn, Mint, Pair as PairContract, Swap, Sync, Transfer } from "../../generated/templates/Pair/Pair";
import { ADDRESS_ZERO, BI_18, FACTORY_ADDRESS, ONE_BI, ZERO_BD } from "../const";
import { updateFactoryDayData, updatePairDayData, updatePairHourData, updateTokenDayData } from "./dayUpdates";
import { findBnbPerToken, getBnbPriceInUSD, getTrackedLiquidityUSD, getTrackedVolumeUSD } from "./pricing";
import { convertTokenToDecimal, createLiquidityPosition, createLiquiditySnapshot, createUser } from "./utils/index";
...
export function handleTransfer(event: Transfer): void {
  // Initial liquidity - TODO if we have the same
  if (event.params.to.toHex() == ADDRESS_ZERO && event.params.value.equals(BigInt.fromI32(1000))) {
    return;
  }
  log.warning("Transfer within {} pair", [event.address.toHex()])
...
export function handleSync(event: Sync): void {
  log.warning("new sync event; pair {}", [event.address.toHex()])
  let pair = Pair.load(event.address.toHex())!;
...
export function handleMint(event: Mint): void {
  ...
  createLiquiditySnapshot(liquidityPosition, event, true)
...
export function handleBurn(event: Burn): void {
  ...
  burn.save();
  log.debug("{}", [burn.sender.toHexString()])
...
export function handleSwap(event: Swap): void {
  log.warning("Swap within {} pair", [event.address.toHex()])
  let pair = Pair.load(event.address.toHex())!;
  let token0 = Token.load(pair.token0)!;
  let token1 = Token.load(pair.token1)!;
  ...
  factoryDayData.dailyVolumeBNB = factoryDayData.dailyVolumeBNB.plus(trackedAmountBNB);
